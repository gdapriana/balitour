...
class CategoryController {
  ...
  static async get(req, res, next) {
    try {
      ...
      const response = await CategoryService.get(slug);
      res.status(200).json({data: response});
    } catch (e) {
      next(e)
    }
  ...
  static async gets(req, res, next) {
    try {
      ...
      const response = await CategoryService.gets(otherQueries);
      res.status(200).json({ data: response });
    } catch (e) {
      next(e)
    }
  ...
  static async update(req, res, next) {
    try {
      const slug = req.params.slug;
      const request = req.body;
      const response = await CategoryService.update(slug, request);
      res.status(200).json({data: response});
    } catch (e) {
      next(e)
    }
