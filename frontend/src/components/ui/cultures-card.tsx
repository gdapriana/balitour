...
const CultureCard = ({culture}: { culture: Culture }) => {
  return (
    <Link className="rounded-[1.3rem] border flex flex-col group overflow-hidden" to={`/destinations/${culture.slug}`}>
      <img src={culture.cover || ""} className="aspect-video object-cover transition duration-300 grayscale group-hover:grayscale-0" />
