...
const RelatedItem = ({ items, itemsName }: { items?: Destination[] | Culture[] | Story[]; itemsName: string }) => {
  return (
    <div className="flex flex-col gap-4 justify-start items-stretch">
      <h1 className="font-bold text-lg">Related {itemsName}</h1>
      <div className="flex flex-col gap-2 justify-start items-stretch">
        {items?.slice(0, 4).map((item: Destination | Culture | Story, index: number) => {
          return (
