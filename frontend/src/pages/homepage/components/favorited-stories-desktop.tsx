        ...
        {stories.map((story: Story, index: number) => {
          return (
            <m.div
              ...
              key={index}
              className={cn("border w-auto overflow-hidden flex justify-start items-center gap-1 p-[4px] rounded-full", activeStory === index ? "pr-4" : "pr-[4px]")}
            >
              <Avatar>
              ...
              {activeStory === index && (
                <m.p animate={{width: ["0%", "100%"], opacity: [0, 1]}} transition={{ease: "anticipate", duration: 0.8}}
                     className="line-clamp-1">{story.username}</m.p>
              )}
            </m.div>
          )
        })}
          ...
          return (
            ...
            >
              <m.img
                ...
                src={story.cover} className={cn("h-full object-cover")}
