        ...
        {stories?.map((story: Story, index: number) => {
          return (
            <m.div
              key={index}
              animate={
                activeStory === index ? { width: "43%" } : { width: "19%" }
              }
              transition={{ duration: 1, ease: "anticipate" }}
              onMouseEnter={() => setActiveStory(index)}
              className={cn(
                "border h-[350px] relative transition duration-1000 overflow-hidden rounded-[1.3rem]",
              )}
            >
              <m.img
                animate={
              ...
              <m.div
                animate={
                  activeStory === index ? { bottom: 0 } : { bottom: "-100%" }
                }
